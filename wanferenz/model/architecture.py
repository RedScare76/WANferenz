...
from wanferenz.kernels.reference import (
    act_quant,
    fp4_act_quant,
    fp8_gemm,
    fp4_gemm,
    ...
    def forward(self, x: torch.Tensor, start_pos: int, main_x: torch.Tensor):
        ...
        if start_pos == 0:
            if seqlen <= win:
                ...
                cutoff = seqlen % win
                self.kv_cache[:bsz, cutoff:win], self.kv_cache[:bsz, :cutoff] = main_kv[
                    :, -win:
                ].split([win - cutoff, cutoff], dim=1)
            return x
...
class Transformer(nn.Module):
    ...
    @torch.inference_mode()
    def forward_spec(
        self, input_ids: torch.Tensor, main_hidden: torch.Tensor, start_pos: int = 0
    ):
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    args = ModelArgs(
        n_hash_layers=0, dspark_block_size=6, dspark_target_layer_ids=(5, 6)
    )
